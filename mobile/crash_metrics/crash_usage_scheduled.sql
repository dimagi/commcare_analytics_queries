--BigQuery query name: CommCare Crash & Usage Metrics : Daily Scheduled Run
--
-- GENERATED by concatenating ga_device_day_insert.sql and
-- crash_usage_history_insert.sql. Edit those, not this file, and regenerate.
--
-- This is the one query to schedule. It refreshes the GA4 rollup and then writes
-- the metrics, in that order, so there is no second job to keep in step. The two
-- halves share a single data_lag_days, so their windows cannot drift apart.
...
-- 1. Refresh the GA4 daily rollup.
...
-- 2. Write the metrics off it. The ASSERT is redundant when step 1 has just run,
-- and kept because it is the thing that catches a day GA4 never delivered.
