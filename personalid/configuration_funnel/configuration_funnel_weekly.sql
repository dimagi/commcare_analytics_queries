...
SELECT
  ...
  SUM(old_sessions) AS old_sessions,
  SUM(recent_sessions) AS recent_sessions,
...
FROM (
  ...
  SELECT
    ...
    COUNT(DISTINCT(older_user_id)) AS old_users,
    COUNT(DISTINCT(recent_user_id)) AS recent_users,
    COUNT(older_session_id) AS old_sessions,
    COUNT(recent_session_id) AS recent_sessions,
    phone_page,
    ...
    photo_page,
    created,
    CASE WHEN COUNT(older_user_id) + COUNT(recent_user_id) = 1 THEN MAX(last_created) ELSE NULL END as last_created,
    CASE WHEN COUNT(older_user_id) + COUNT(recent_user_id) = 1 THEN MAX(last_recovered) ELSE NULL END as last_recovered,
    CASE WHEN COUNT(older_user_id) + COUNT(recent_user_id) = 1 THEN MAX(last_lockout) ELSE NULL END as last_lockout,
    CASE WHEN COUNT(older_user_id) + COUNT(recent_user_id) = 1 THEN MAX(last_wrong_backup_code) ELSE NULL END as last_wrong_backup_code
  FROM
  (
    SELECT
    user_pseudo_id,
    ...
    CASE WHEN recent_data = 0 THEN user_pseudo_id ELSE NULL END AS older_user_id,
    CASE WHEN recent_data = 1 THEN user_pseudo_id ELSE NULL END AS recent_user_id,
    CASE WHEN recent_data = 0 THEN ga_session_id ELSE NULL END AS older_session_id,
    CASE WHEN recent_data = 1 THEN ga_session_id ELSE NULL END AS recent_session_id,
    ...
    GROUP BY user_pseudo_id, recent_data, ga_session_id
  )
  GROUP BY
    phone_page,
    start_configuration_integrity_device_failure,
